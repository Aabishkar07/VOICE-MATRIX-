    ...
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-x-4">
            <div>
                ...
                <p class="text-sm text-gray-600">Manage all your team members</p>
            </div>
        </div>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-500">ID: #{{ $teams->id }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                @if ($teams->image)
                                    <div class="flex-shrink-0">
                                        <img class="w-16 h-16 rounded-lg object-cover shadow-sm border border-gray-200"
                                            src="{{ asset('/uploads/' . $teams->image) }}" alt="{{ $teams->name }}">
                                    </div>
                                @else
        ...
        function deleteItem(itemId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let form = document.querySelector('#delete-form-' + itemId);
                    form.submit();
                }
            });
        }
