...
@section('body')
    ...
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-x-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Affiliations</h1>
                <p class="text-sm text-gray-600">Manage all your affiliations</p>
            </div>
        </div>

        <div class="flex items-center gap-3">
            <a href="{{ route('admin.affiliations.create') }}"
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($affiliations as $key => $affiliation)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 w-2 h-2 bg-green-500 rounded-full mr-3"></div>
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">{{ $affiliation->title }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                    ...
                                    @if ($affiliation->image)
                                        <div class="flex-shrink-0">
                                            <img class="w-12 h-12 rounded-lg object-cover shadow-sm border border-gray-200"
                                                src="{{ asset('uploads/' . $affiliation->image) }}" alt="{{ $affiliation->title }}">
                                        </div>
                                    @else
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">{{ $affiliation->created_at->format('M d, Y') }}</div>
                                <div class="text-sm text-gray-500">{{ $affiliation->created_at->format('g:i A') }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    <a href="{{ route('admin.affiliations.edit', $affiliation->id) }}"
                                    ...
                                    <form method="POST" action="{{ route('admin.affiliations.destroy', $affiliation->id) }}"
                                        id="delete-form-{{ $affiliation->id }}" class="inline">
                                        @csrf
                                        @method('delete')
                                        <button type="button" onclick="deleteItem({{ $affiliation->id }})"
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900 mb-2">No affiliations found</h3>
                                    <p class="text-gray-500 mb-4">Get started by creating your first affiliation.</p>
                                    <a href="{{ route('admin.affiliations.create') }}"
