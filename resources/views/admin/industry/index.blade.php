...
@section('body')
    ...
    <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-x-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Insustry We Serve</h1>
                <p class="text-sm text-gray-600">Manage all your industries </p>
            </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($industries as $key => $industry)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-500">ID: #{{ $industry->id }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                @if ($industry->image)
                                    <div class="flex-shrink-0">
                                        <img class="w-16 h-16 rounded-lg object-cover shadow-sm border border-gray-200"
                                            src="{{ asset('uploads/' . $industry->image) }}" alt="{{ $industry->title }}">
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">{{ $industry->created_at->format('M d, Y') }}</div>
                                <div class="text-sm text-gray-500">{{ $industry->created_at->format('g:i A') }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.industries.destroy', $industry->id) }}"
                                        id="delete-form-{{ $industry->id }}" class="inline">
                                        @csrf
                                        @method('delete')
                                        <button type="button" onclick="deleteItem({{ $industry->id }})"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 mb-4">Get started by creating your first industry.</p>
                                    <a href="{{ route('admin.industries.create') }}"
