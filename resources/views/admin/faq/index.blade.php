...
@section('body')
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-sm text-gray-600">Manage frequently asked questions</p>
        </div>
        <div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach ($faqs as $key => $faq)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900">{{ $faq->title }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">{{ $faq->description }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900">{{ $faq->created_at->format('M d, Y') }}</div>
                                <div class="text-sm text-gray-500">{{ $faq->created_at->format('g:i A') }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.faqs.destroy', $faq->id) }}" id="delete-form-{{ $faq->id }}" class="inline">
                                        @csrf
                                        @method('delete')
